...
NFR-13 requires a minimal dependency surface. The package needs state machines (Phase 7),
...
designed in Phase 7 as a minimal internal component scoped to invariants I-03/I-04
(guarded transitions, actor attribution, event + audit emission) — not a general-purpose
state machine library.

`spatie/laravel-model-states` (2) is rejected for v1: it is class-per-state oriented,
brings its own extension model that would compete with ours (FR-903), and adds a version
...
- **+** Consumers inherit no transitive constraints beyond the framework; Laravel
  11/12/13 matrix stays ours to manage (NFR-02).
- **+** Transition semantics exactly match invariants I-03/I-04 — nothing unused, nothing
  missing.
- **−** We own ~one small internal mechanism (design cost in Phase 7, test cost per
  NFR-07) — accepted; the mechanism is internal, so we can change it without BC impact as
  long as configured workflows (FR-903) keep working.
